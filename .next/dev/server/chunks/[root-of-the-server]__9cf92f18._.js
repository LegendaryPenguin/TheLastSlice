...
"[project]/src/app/api/raid/start/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(req) {
    ...
    const { data: raid, error: raidErr } = await sb.from("raids").select("*").eq("code", code).single();
    ...
    if (raid.status !== "lobby") return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        error: "Raid not in lobby."
    }, {
        status: 409
    });
    const startedAt = new Date();
    const endsAt = new Date(startedAt.getTime() + raid.duration_seconds * 1000);
    const { data: updated, error } = await sb.from("raids").update({
        status: "live",
        started_at: startedAt.toISOString(),
        ends_at: endsAt.toISOString()
    }).eq("id", raid.id).select("*").single();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__9cf92f18._.js.map