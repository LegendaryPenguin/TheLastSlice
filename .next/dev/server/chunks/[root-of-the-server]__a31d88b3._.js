...
"[project]/src/app/api/raid/state/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(req) {
    ...
    const { data: players } = await sb.from("players").select("*").eq("raid_id", raid.id).order("total_damage", {
        ascending: false
    });
    const { data: attacks } = await sb.from("attacks").select("*").eq("raid_id", raid.id).order("created_at", {
        ascending: false
    }).limit(200);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a31d88b3._.js.map